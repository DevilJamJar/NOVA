...
class Image(commands.Cog):
    ...
    async def pixelate(self, ctx, member: discord.Member = None):
        """Add a pixelated filter to a profile picture."""
        user = member or ctx.message.author
        av = str(user.avatar_url_as(format='png'))
        url = f'https://some-random-api.ml/canvas/pixelate?avatar={av}'
        embed = discord.Embed(color=0x5643d, timestamp=ctx.message.created_at)
        embed.set_image(url=url)
        await ctx.send(embed=embed)
